...
### Completed (goal)
- Tutti item in sospeso terminati:
  - Catalogo hub community portato a 15 package.
  - Secondo runner E2E fisico + soak.
  - HOMEBREW_TAP_TOKEN.
- **Full codebase review + standalone DMG**:
  - Nessuna dipendenza Python per utenti che scaricano il DMG.
  - Nuova embed-distribution-python.sh che scarica e include un Python runtime completo (python-build-standalone).
  - distribution-launcher.sh ora usa esclusivamente l'interprete embedded.
  - Aggiornati runtime detection, permissions, messages, repair, verify, build scripts per promuovere "Expando.app" standalone.
  - Bundle DMG ora veramente self-contained, pulito, leggero (solo codice necessario + embedded python).
- verify_goal_ac.sh e refactor ACs solidi.
- docs aggiornati.
- Risolti tutti i punti critici scoperti nella review (bundle detection fragility, sys.executable fallbacks, bare excepts in doctor_repair/daemon/listener, shell=True, legacy "python" grants, brittle embed/repair scripts, dist mismatch):
  - Unificato rilevamento app bundle in `paths.find_expando_app_bundle()` (env + sys.executable + __file__ + sibling/cwd lookup per dev .app).
  - `runtime_info.detect_runtime`, `daemon._app_bundle_executable`, `ui_bridge._ui_subprocess_argv` usano la logica unificata e preferiscono launcher/embedded per mode=app.
  - Grant label "Expando.app" per app mode; "python" solo per dev/legacy TCC.
  - `injector.py`: rimosso shell=True su Windows clipboard (usa list argv).
  - Script: distribution-launcher strict su embedded, embed con PY_ARCH dinamico, repair prefer embedded, verify/build strict reject dev-venv.
  - Narrow bare `except Exception:` in doctor_checks, doctor_repair (clear_safe_mode ora con logger.exception), daemon restart, listener recovery, paths, permissions.
  - Test aggiornati per guidare il codice reale spedito (no bypass su _app_bundle_executable, find, detect, repair_daemon_state).
  - Verification plan rieseguito letteralmente (core_functions, fix_pytest_core 34 pass, cli, shell, doctor_units, evidence); tutte le gate pass con prove in SCRATCH.
