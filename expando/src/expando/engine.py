...
class ExpansionEngine:
    ...
    def _collect_candidates(
        ...
        *,
        ...
        """Return raw (trigger, match) pairs that match the current buffer
        syntactically (endswith + ignore_case) and satisfy word-break rules.
        Does NOT apply app/when allowed checks.
        Single helper per plan: literal+regex + word rules in ONE place.
        _find_match and _find_blocked_match now delegate (literal loop body appears once).
        """
    ...
    def _find_match(
        ...
        *,
        ...
        raw = self._collect_candidates(
            require_word_break=require_word_break,
            context=context,
            config=config,
        )
        candidates: list[tuple[str, Match]] = []
        for trigger, match in raw:
