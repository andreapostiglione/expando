...
def _client_matches_runtime(client: str, runtime: RuntimeInfo) -> bool:
    ...
    # Legacy fallbacks ONLY for dev/venv/unknown (pre-standalone DMG installs)
    # App mode must match Expando.app exactly (no "python" grant for users)
    if runtime.mode != "app":
        labels.update({"python", "python3"})
    ...
    *,
    ...
    if platform.system() == "Darwin" and runtime is not None:
        if runtime.mode == "app":
            ...
            if runtime.mode != "app" and "python" in runtime.grant_label.lower():
                notes.append(
                    "Per build standalone (DMG) assicurati di usare la versione embedded. "
                    "Reinstalla l'ultima release DMG se necessario."
                )
