...
if [[ "${EXPANDO_DISTRIBUTION:-0}" == "1" ]]; then
  # DISTRIBUTION = fully standalone DMG. Embeds its own Python runtime (python-build-standalone).
  # End users need NOTHING else installed (no system Python visible). Double-click and go.
  # Detect/reject dev-venv for dist intent.
  if [[ -d "$RESOURCES/venv" && ! -d "$RESOURCES/python" ]]; then
    echo "ERROR: refusing to build distribution artifact from dev-venv layout (use embed path)" >&2
    exit 1
  fi
  ...
  # DEV build: uses local python/venv for fast iteration (not for shipping)
  VENV="$RESOURCES/venv"
  rm -rf "$VENV"
  python3 -m venv "$VENV"
...
if [[ "${EXPANDO_DISTRIBUTION:-0}" != "1" ]]; then
  echo "DEV build: grant Accessibility/Input Monitoring to Expando.app or your python process."
else
  echo "DISTRIBUTION build: standalone DMG. Users grant permissions to Expando.app (no Python install needed)."
fi